...
const SwrInitor = ({
  ...
  useEffect(() => {
    const lastLoginToken = globalThis.localStorage?.getItem('last_login_token')
    if (token && (token !== accessTokenFromLocalStorage) && (token !== lastLoginToken)) {
      globalThis.localStorage?.setItem('last_login_token', token)
