...
const baseFetch = (url: string, fetchOptions: any, { needAllResponseContent, bodyStringify = true, deleteContentType }: IOtherOptions) => {
  const options: typeof baseOptions & FetchOptionType = { ...baseOptions, ...fetchOptions }
    ...
    new Promise((resolve, reject) => {
      globalThis.fetch(urlWithPrefix, options as RequestInit)
        .then((res: any) => {
          ...
          // return data
          const data = options.headers.get('Content-type') === ContentType.download || options.headers.get('Content-type') === ContentType.audio ? res.blob() : res.json()
