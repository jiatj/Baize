...
export const useImageFiles = () => {
  ...
  const handleUpload = (imageFile: ImageFile, callBack?: (hasImage: boolean) => void | undefined) => {
    ...
    if (index > -1) {
      ...
      callBack && callBack(newFiles.length > 0)
      filesRef.current = newFiles
    }
  }
  const handleRemove = (imageFileId: string, callBack?: (hasImage: boolean) => void | undefined) => {
    const files = filesRef.current
