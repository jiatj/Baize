...
import VideoPlus from '@/app/components/base/icons/line/video-plus'
...
import type { VideoFile, VisionSettings } from '@/types/app'

type UploadOnlyFromLocalProps = {
  onUpload: (imageFile: VideoFile) => void
  disabled?: boolean
  limit?: number
}
    ...
    <Uploader onUpload={onUpload} disabled={disabled} limit={limit}>
      {
        hovering => (
          ...
          `}>
            <VideoPlus className='w-4 h-4 text-gray-500' />
          </div>
        )
      }
    </Uploader>
...
const UploaderButton: FC<UploaderButtonProps> = ({
  ...
  const handleUpload = (videoFile: VideoFile) => {
    setOpen(false)
    onUpload(videoFile)
  }
    ...
    >
      ...
      <PortalToFollowElemContent className='z-50'>
        <div className='p-2 w-[260px] bg-white rounded-lg border-[0.5px] border-gray-200 shadow-lg'>
          <VideoLinkInput onUpload={handleUpload} />
          {
            hasUploadFromLocal && (
              <>
                ...
                <Uploader onUpload={handleUpload} limit={limit}>
                  {
                    hovering => (
                      ...
                      `}>
                        ...
                        {t('common.videoUploader.uploadFromComputer')}
                      </div>
                    )
                  }
                </Uploader>
...
type ChatVideoUploaderProps = {
  settings: VisionSettings
  onUpload: (videoFile: VideoFile) => void
  disabled?: boolean
}
const ChatVideoUploader: FC<ChatVideoUploaderProps> = ({
  settings,
  onUpload,
  disabled,
}) => {
...
export default ChatVideoUploader
